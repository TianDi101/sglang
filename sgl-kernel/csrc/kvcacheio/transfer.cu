...
#else
// ROCm: widen the per-lane streaming copy to 128-bit when both operands are
// 16B-aligned. This halves the load/store instruction count and raises the
// per-warp memory-level parallelism used to hide Infinity-Fabric latency on
// L2(host)->L1(device) gathers, so the kernel reaches the fabric bandwidth
// ceiling with fewer CUs (lower block_quota) stolen from compute. Measured on
// MI355X (gfx950), 1024 pages x 61 layers x 1152B MLA gather: 128-bit is
// +32-44% at block_quota<=8 and +36% for device-resident (D2D) sources, and a
// wash once block_quota already saturates the host fabric (~55 GB/s). Operands
// that are not 16B-aligned (or the trailing 8B when item_size_bytes is not a
// multiple of 16) fall back to the original 64-bit path.
...
#if defined(USE_ROCM)
  // Optional HIP batch path mirroring the NVIDIA cudaMemcpyBatchAsync path:
  // assemble every per-page/per-layer copy and submit them in a single
  // hipMemcpyBatchAsync so the batch API can cut per-copy launch overhead and
  // raise host->device load bandwidth.  DEFAULT DISABLED (kEnableHipBatch ==
  // false) so this falls through to the per-page copy below and behavior is
  // unchanged unless explicitly enabled and rebuilt.  ROCm's per-copy `attrs`
  // argument is currently unsupported, so we pass none; if the runtime rejects
  // the call we fall back to the per-page copy.
